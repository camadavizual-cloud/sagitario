...
function billingType(row: Row): "monthly" | "one_time" | "setup" {
  ...
  if (["monthly", "mensal", "recurring", "recorrente", "subscription", "assinatura", "recurring_monthly", "monthly_subscription"].includes(raw)) return "monthly";
  if (["setup", "initial", "taxa_inicial", "implantacao", "implantação", "activation", "ativacao", "ativação", "initial_fee", "setup_fee"].includes(raw)) return "setup";
  return "one_time";
}
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ niches, services, company }, { headers: { "Cache-Control": "no-store", "X-Sagitario-Build": "niches-services-v2-clean", "X-Sagitario-Niche-Columns": Object.keys(nicheRows[0] || {}).sort().join(","), "X-Sagitario-Service-Columns": Object.keys(serviceRows[0] || {}).sort().join(",") } });
  } catch (error) {
    ...
    return NextResponse.json(
      ...
      },
      {
        ...
        headers: {
          ...
          "X-Sagitario-Build": "supabase-key-normalization-v3",
        },
      },
    );
  }
