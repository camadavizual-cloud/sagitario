...
const isContractNiche = (name?: string) => {
  const normalized = (name || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLocaleLowerCase("pt-BR");
  return /\bclinicas?\b|\brestaurantes?\b/.test(normalized);
};
...
export default function Home() {
  ...
  const contractTermsEnabled = isContractNiche(currentNiche?.name);
